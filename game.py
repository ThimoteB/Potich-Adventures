...
class Game:
    ...
    def remove_fog(self, cell: list):
        ...
        Args:
            cell (list): list of cells
        """
        self.list_fog_cases = [case for case in self.list_fog_cases if case not in cell]
    ...
    def run(self):
        ...
        # Management of the fog
        if self.fog:
            for pawn in self.list_pawns:
                self.remove_fog(self.get_nearby_cells(pawn, self.fog_range))
                        ...
                        if cell.game_object and isinstance(cell.game_object, Pawn):
                            if cell.is_heal_cell:
                                ...
                                if cell.game_object.health != old_health:
                                    self.tab.log_event.write_logfile(
                                        "log_event.txt",
                                        f"{cell.game_object.name} a été \
                                            soigné de {cell.heal_value} HP",
                                    )
                                if SOUND:
                                    if cell.game_object.health != old_health:
                    ...
                    # Case where the click is on the board
                        ...
                        # Search if the clicked cell is in the highlighted cells
                            ...
                            if (clicked_cell_y, clicked_cell_x) in highlighted_cells:
                                ...
                                # Remove the fog around the pawn
                                self.remove_fog(self.get_nearby_cells(pawn_selected, 3))
